...
class Program 
{
    static void Main(string[] args)
    {
        ...
        collection[5] = new JellyBean("Cinnamon");
        collection[6] = new JellyBean("Coconut");

        JellyBeanIterator it = collection.CreateIterator();

        Console.WriteLine("Gimme all the jelly beans!");

        for (JellyBean jb = it.First();
            !it.IsDone; jb = it.Next())            
        {
            Console.WriteLine(jb.Flavour);
        }
